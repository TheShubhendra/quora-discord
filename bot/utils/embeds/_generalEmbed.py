...
def feedbackEmbed(client: commands.Bot,
                  interaction_user: discord.Member,
                  feedback: str) -> discord.Embed:
    ...
    embed.add_field(name="Feedback Recieved",
                    value=feedback)
...
def pingEmbed(client: commands.Bot, interaction_user: discord.Member, latency: int) -> discord.Embed:
    embed = discord.Embed(
        title=f"Pong! {latency}ms",
...
def registerEmbed(client: commands.Bot, interaction_user: discord.Member, username: str, found: bool) -> discord.Embed:
    if found is None:
    ...
    embed.set_author(name=f"Quora Bot", icon_url=client.user.avatar.url)
    if found:
        embed.add_field(name="To View Profile",
                        value='Use /profile')
    embed.timestamp = datetime.utcnow()
    embed.set_footer(
        text=interaction_user.display_name,
